...
def make_taxo_table(args):
    ...
    def decom_taxa_strs(tax_list):
        tax_dict = { t.split(":")[0] : t.split(":")[1].replace("(NOR5)", "_NOR5_").replace(")","").split("(")  for t in tax_list}
        tax_dict = {k : {'taxon' : v[0].replace('"', ''), 'score' : float(v[1]) } for k,v in tax_dict.items() if len(v) > 1}
        return tax_dict

    get_tail = lambda tax_dict, thresh :  ";".join([ tax_dict[c]['taxon'] for c in order if tax_dict.has_key(c) and tax_dict[c]['score']  > thresh])
    get_level = lambda tax_dict, thresh, level : tax_dict[level]['taxon'] if tax_dict.has_key(level) and tax_dict[level]['score']  > thresh else None
...
#    out_dir = "test"
    ...
    table['taxa']= Series(taxa_dicts["tail"])
...
def prefilter_hgca_table_and_stuff(args):
    table = "/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/otu_table_fwd.csv"
    classif = "/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/classification.txt"
    centers = "/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/centers_fwd.fasta"
    ...
    with open("/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/centers_stump.fasta","w") as handle:
        SeqIO.write(sub_seqs,handle,"fasta")

    with open("/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/classification_stump.txt","w") as handle:
        handle.writelines(sub_class_lines)

    sub_table.to_csv("/home/moritz/Data/people/0002_stump/00A_hgcA_amplicon/otu_table_stump.csv")
